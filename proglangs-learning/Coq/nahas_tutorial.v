...
Proof.
  ...
  intros proof_of_A.
  exact proof_of_A.
  (*apply proof_of_A. [> Yet another way to do the same <]*)
Qed.
...
refine
    (fun x:Option =>
      ...
      | Fail => _  (* This is the empty space in this expression *)
      | Ok b => fun _ => b (* This empty space corresponds only to the function defined the function *)
      end).
intros. absurd (Fail = Fail). trivial. trivial.
(*intros. absurd (Fail = Fail); trivial. [> same as above O_O <]*)
Defined.

Compute get Fail _. (* This is nothing!! *)
Check get Fail _.
Compute get (Ok true) _. (* This is true *)
Check get (Ok true) _.
Check fun _ => 3.
...
Proof.
 ...
 (*refine (A_implies_B _).*)
 (*exact proof_of_A.*)
 apply (B_implies_C (A_implies_B proof_of_A)). (* this is simpler in this case *)
Qed.
...
Proof.
 ...
 refine (A_imp_B_imp_C _ _).
   ...
   refine (A_implies_B _).
     exact proof_of_A.
 (* or *)
 (*apply (A_imp_B_imp_C proof_of_A (A_implies_B proof_of_A)).*)
Qed.
...
Proof.
  (*intros n; apply n. [> This is the whole, proof. In agda it would be f bot = bot <]*)
  unfold not. (* This helps us unroll the not definition, just to make it clearer when running line by line *)
...
Theorem absurdanything : forall C : Prop, False -> C.
Proof.
  intros c.
  intros f.
  case f. (* "case" works in here like the absurd pattern works in Agda *)
Qed.
...
Theorem absurd2_meine : forall A C : Prop, A -> ~ A -> C.
Proof.
  intros A C a not_a.
  exact (absurdanything _ (not_a a)).
Qed.
...
Proof.
  simpl.  (* Nice, this little thing can simplify to normal form :) *)
  exact I.
Qed.
...
Proof.
  ...
  refine (conj _ _).
    case a; simpl.
      intros aorb; exact (or_introl I).
      case b; simpl;
        intros aorb; exact (or_intror aorb).
    case a; simpl.
      intros TOR_; exact I.
      case b; simpl.
      intros FORF; case FORF; intros F; exact I.
      intros FORF; case FORF; intros F; exact F.
...
Proof.
  intros a; case a; simpl; refine (conj _ _).
    intros f t; exact f.
    intros notT; exact (notT I).
    intros t f; exact f.
    intros notf; exact I.
(*
...
Print ex.
Check ex_intro basic_predicate.
Check ex_intro basic_predicate true.
Check ex_intro basic_predicate true I.

Theorem thm_exists_basics : (ex basic_predicate).
Proof.
  (*apply (ex_intro basic_predicate true I). [> short version <]*)
  pose (witness := true).
...
Proof.
  intros b; case b; simpl.
    refine (ex_intro _ true _); exact I.
    refine (ex_intro _ false _); exact I.
(*
 *  intros b.
 *  case b.
 *    [>* b is true <]
...
Proof.
  ...
  case b; exact I.
  (*exact (eqb_a_a b). [> original line used for the thing above <]*)
Qed.
...
Proof.
  (*My version*)
  intros p forall_x_not_Px exists_x_Px.
  destruct exists_x_Px as [x proof_Px].
  apply (forall_x_not_Px x proof_Px).
...
Proof.
  (* My version *)
  intros P not_exists_x_Px x Px.
  pose (exists_x_Px := ex_intro P x Px).
  exact (not_exists_x_Px exists_x_Px).
...
Proof.
  (* My version *)
  intros a b.
  case a, b; simpl; exact (eq_refl _).
(*
 *  intros a b.
 *  case a, b.
 *    [>* suppose a,b is true,true <]
...
Proof.
  intros a.
  case a; intros a_eq_neg_a; discriminate a_eq_neg_a.
(*
 *  intros a.
 *  unfold not.
 *  case a.
...
Theorem plus_n_O_mine : (forall n, n + 0 = n).
Proof.
  intros n.
...
(*
 *Definition f_equal_this : forall (A B : Type) (f : A -> B) (x y : A), x = y -> f x = f y :=
...
(* YES!!! Skipping tactics and doing it à la Agda is possible, albeit Coq
* help on this kind of proof are extremely limited. Really good support
* for tactics, not great for things a la Agda *)
Fixpoint plus_n_O_mine_fun (n: nat) : (n + 0 = n) :=
  match n with
  | O => eq_refl
  | S n' => f_equal S (plus_n_O_mine_fun n')
  end.
...
Proof.
  ...
  induction n.
    ...
    simpl.
    rewrite IHn.
    (* It's superweird, but `induction` is not the same `elim` + `intros`
    *  `induction` creates a function for `IHm'` not a hypothesis *)
...
Definition correctness_of_hd_error_fun :
   (forall A:Type,
   (forall (x : A) (lst : list A),
   (hd_error A nil) = None /\ (hd_error A (x :: lst)) = Some x)) :=
   (fun (A : Type) (x : A) (lst : list A) => conj eq_refl eq_refl).
